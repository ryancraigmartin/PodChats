...
router.get('/', async (req, res) => {
	try {        // ^ Specifies that the anonymous (req, res) function will be asynchronous. 
			...
			.then((thePodcast)=> {
				...
				.then(res => {
					...
					xmlParser(res.data, function(error, feedResult) {
						feedResult.rss.channel[0].item.forEach(function(episode, index) {
							if (index < 15){
					...
					return res.status('404').json({error: `No results for search: ${req.query.term}`})
				} // END PODCAST IF/ELSE
